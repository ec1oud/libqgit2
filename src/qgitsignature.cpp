...
{
    git_signature *sig = 0;
    qGitThrow(git_signature_new(&sig, name.toUtf8(), email.toUtf8(), dateTime.toSecsSinceEpoch(), dateTime.offsetFromUtc() / 60));
...
QDateTime Signature::when() const
{
    QDateTime dt;
    if (d_ptr->signature) {
        dt.setSecsSinceEpoch(d_ptr->signature->when.time);
